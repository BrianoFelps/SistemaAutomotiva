...
public class DaoPrSr extends BancoDeDadosMySQL{
    ...
    try{
        sql = "UPDATE PRODUTOSSERVICOS (ID, IDGRUPO, NOME, DESCRICAO, PRECO) VALUES (?, ?, ?, ?, ?)";
        
        setStatement(getConexao().prepareStatement(sql));
        
        getStatement().setInt(1, id);
        getStatement().setInt(2, nIdGp);
        getStatement().setString(3, nNome);
        getStatement().setString(4, nDesc);
        getStatement().setDouble(5, nPreco);
